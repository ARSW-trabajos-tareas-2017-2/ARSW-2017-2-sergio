...
public class HostBlackListsValidator {
    ...
    private static final int MAX_hilos =8;
    ...
    public List<Integer> checkHost(String ipaddress) {
        ...
        int current_server = 0; // cuantos servidores he revisado

        ArrayList<tarea> tareas = new ArrayList<>();

        // crearción y ejecucuión
        for (int i = 0; i < servidores && ocurrencesCount < BLACK_LIST_ALARM_COUNT; i++) {

            for (int s = 0; s < MAX_hilos && s + current_server < servidores; s++) {

                tareas.add(new tarea(skds, s + current_server, ipaddress));
                tareas.get(s + current_server).start();
            }

            // esperar a los demas hilos
            for (int s = 0; s < MAX_hilos && s + current_server < servidores; s++) {

                checkedListsCount++;
                
                try {
                    tareas.get(s + current_server).join();
                } catch (InterruptedException ex) {
                    Logger.getLogger(HostBlackListsValidator.class.getName()).log(Level.SEVERE, null, ex);
                }

                if (tareas.get(s + current_server).isIsBlack()) {
        ...
        LOG.log(Level.INFO, "Checked Black Lists:{0} of {1}", new Object[]{checkedListsCount,
